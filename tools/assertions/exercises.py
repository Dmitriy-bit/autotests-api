...
def assert_create_exercise_response(
    ...
    """
    Проверяет, что ответ запроса на создание задания соответствует данным отправленным в запросе.
...
def assert_get_exercise_response(get_exercise_response: GetExerciseResponseSchema,
    ...
    :param create_exercise_response: API ответа при создании задания.
...
def assert_update_exercise_response(request: UpdateExerciseRequestSchema,
    ...
    :param response: Ответ API с обновлёнными данными.
    :raises AssertionError: Если хотя бы одно поле не совпадает.
    """
    assert_equal(response.exercise.title, request.title, "title")
    assert_equal(response.exercise.max_score, request.max_score, "max_score")
